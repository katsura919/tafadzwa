...
const logoGroups = receipts.map((r) => [
  <div key={r.label} className="flex flex-col items-center text-center">
    <span className="font-heading text-xl font-bold tracking-tight text-white sm:text-3xl md:text-5xl">
      <Counter to={r.value} suffix={r.suffix} />
    </span>
    <span className="mt-2 text-[11px] font-medium text-white sm:text-[13px]">{r.label}</span>
  </div>,
])
  ...
  return (
    <section className="px-4">
      <Reveal className="flex flex-col items-center">
        ...
        <div className="mx-auto w-fit overflow-hidden rounded-3xl border border-primary bg-primary shadow-[0_18px_40px_-16px_rgba(139,92,246,.45)]">
          <StackedLogos
            logoGroups={logoGroups}
            logoWidth="clamp(76px, 22vw, 210px)"
            lineClassName="bg-gold/40"
          />
        </div>
      </Reveal>
    </section>
